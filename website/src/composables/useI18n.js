...
export const japanesePluginDescriptions = {
  ...
  steprndflt: "テンポ同期のサンプル＆ホールドLFOをマルチモードTPT SVFのカットオフへかけます。各ステップでランダム値を引き直し、Xは深さ、Yはレゾナンス（上限付き）。TYPEは各モードに*R（ステップごとのフィルタ混ぜ具合S&H）を併記（初期Peak）。LEVELでウェット→最終ソフトクリップ。パッドを押している間だけ効きます。",
