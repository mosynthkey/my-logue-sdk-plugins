...
static void render(Dembow &kit, std::vector<float> &mono, uint32_t frames)
{
...
static void setup(Dembow &kit, int32_t rim, int32_t perc)
{
  kit.init(nullptr);
  kit.setParameter(Dembow::MIX, 1000);
  kit.setParameter(Dembow::RIM, rim);
  kit.setParameter(Dembow::PERC, perc);
  kit.setParameter(Dembow::SWING, 360);
  kit.setParameter(Dembow::TONE, 510);
  kit.setParameter(Dembow::DEC, 410);
  kit.setTempo(96.f);
}
...
int main()
{
  ...
  // Kick spine 0/6/8/14 + snare 4/12 + cha 7/15.
  ...
  kit.debugTriggerStep(14U);
  kit.debugTriggerStep(15U);
  if (kit.debugMainTriggers() < 8U)
  {
  ...
  {
    std::printf("rim triggers too low: %u\n", kit.debugGhostTriggers());
    return 2;
  }

  const uint32_t high_rims = kit.debugGhostTriggers();
  setup(kit, 80, 200);
  kit.debugResetCounters();
  for (uint32_t passIndex = 0; passIndex < 8U; ++passIndex)
  {
  ...
  const uint32_t low_rims = kit.debugGhostTriggers();
  if (low_rims >= high_rims)
  {
    std::printf("low rim=%u not below high=%u\n", low_rims, high_rims);
    return 3;
  }
  ...
  mono.clear();
  const uint32_t bar_samples = static_cast<uint32_t>(48000.f * 60.f / 96.f * 4.f);
  ...
  kit.touchEvent(0, k_unit_touch_phase_began, 400U, 400U);
  kit.tempo4ppqnTick(7U); // step 6 = kick
  ...
  std::printf("ok rims_high=%u rims_low=%u kick_peak=%f late_ratio=%f bar_peak=%f\n", high_rims, low_rims,
