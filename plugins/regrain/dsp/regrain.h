 ...
 *
 * Always runs a Schroeder tank on input (raw preferred). The wet reverb
 * output is what gets recorded into SDRAM. Touch freezes that buffer and
 * plays a tempo-aware grain cloud. Untouched = dry bypass.
 *
 * X/FEEL = grain density. Y/SIZE = reverb decay depth.
 * ENV = grain attack/release. TONE = reverb damping. Prefers get_raw_input.
 */
...
public:
  ...
  static constexpr float kMaxGrainMs = 100.f;
  static constexpr float kMinBpm = 40.f;
  static constexpr float kMaxBpm = 300.f;
  static constexpr float kMinCapturePeak = 0.002f;
  ...
  {
    return kMaxCaptureSamples * 2U + kCombLineCount * kCombSize + kAllpassLineCount * kAllpassSize;
  }
  ...
  void setParameter(uint8_t index, int32_t value) override final
  {
    ...
    case FEEL:
      ...
      break;
    case SIZE:
      size_norm_ = param10BitToNorm(value);
      updateReverbParams();
      break;
    case MIX:
      mix_ = value / 1000.f;
    ...
    case SPRD:
      ...
      break;
    case TONE:
      tone_norm_ = param10BitToNorm(value);
      updateReverbParams();
      break;
    case REVS:
  ...
  void init(float *allocated_buffer) override final
  {
    ...
    float *cursor = allocated_buffer + kMaxCaptureSamples * 2U;
  ...
  void reset() override final
  {
    ...
    spawn_countdown_ = 0.f;
    rng_state_ = 0xA11CE55U;
    rev_write_ = 0U;
    for (uint32_t combIndex = 0; combIndex < kCombLineCount; ++combIndex)
      damp_states_[combIndex] = 0.f;
    clearGrains();
    clearReverb();
  }
  ...
  void touchEvent(uint8_t id, uint8_t phase, uint32_t x, uint32_t y) override final
  {
    ...
    size_norm_ = static_cast<float>(y) * (1.f / 1023.f);
    updateSpawnPeriod();
    updateReverbParams();
  ...
  void process(const float *__restrict in, const float *__restrict raw, float *__restrict out, uint32_t frames)
  {
    for (uint32_t sampleIndex = 0; sampleIndex < frames; ++sampleIndex)
    {
      ...
      float rev_left = 0.f;
      float rev_right = 0.f;
      processReverb(src_left, src_right, rev_left, rev_right);

      if (!frozen_)
      {
        // Capture the deep wet reverb, not the dry source.
        recordSample(rev_left, rev_right);
...
private:
  ...
  {
    // Reverb needs a little time to bloom before freeze is useful.
    uint32_t ready = static_cast<uint32_t>(getSampleRate() * 0.35f);
  ...
  {
    const float size = clamp01(size_norm_);
    // Deep tank by default; Y pushes feedback toward near-infinite wash.
    comb_feedback_ = 0.84f + size * 0.14f;
    const float tone = clamp01(tone_norm_);
    // Higher TONE → faster one-pole → brighter; lower → darker.
    damp_coeff_ = 0.06f + tone * 0.5f;
  }
  ...
  void clearReverb()
  {
    ...
    for (uint32_t combIndex = 0; combIndex < kCombLineCount; ++combIndex)
    {
      if (comb_lines_[combIndex] == nullptr)
        continue;
    ...
    for (uint32_t allpassIndex = 0; allpassIndex < kAllpassLineCount; ++allpassIndex)
    {
      if (allpass_lines_[allpassIndex] == nullptr)
        continue;
  ...
  {
    static const uint32_t kDelays[kCombLineCount] = {2521U, 2765U, 3011U, 3259U};
    float *line = comb_lines_[combIndex];
    const uint32_t delay = kDelays[combIndex];
    const uint32_t read_index = (rev_write_ + kCombSize - delay) & kCombMask;
    const float delayed = line[read_index];
    float &lp = damp_states_[combIndex];
    lp += damp_coeff_ * (delayed - lp);
    line[rev_write_ & kCombMask] = input + lp * comb_feedback_;
    return lp;
  }

  float processAllpass(uint32_t allpassIndex, float input)
  {
    static const uint32_t kDelays[kAllpassLineCount] = {727U, 947U};
    static constexpr float kAllpassGain = 0.5f;
    float *line = allpass_lines_[allpassIndex];
    const uint32_t delay = kDelays[allpassIndex];
    const uint32_t read_index = (rev_write_ + kAllpassSize - delay) & kAllpassMask;
    const float delayed = line[read_index];
    const float output = -kAllpassGain * input + delayed;
    line[rev_write_ & kAllpassMask] = input + kAllpassGain * delayed;
    return output;
  }

  void processReverb(float left_in, float right_in, float &left_out, float &right_out)
  {
    if (comb_lines_[0] == nullptr)
    {
      left_out = 0.f;
      right_out = 0.f;
      return;
    ...
    const float mono_in = (left_in + right_in) * 0.5f;
    float sum = 0.f;
    for (uint32_t combIndex = 0; combIndex < kCombLineCount; ++combIndex)
      sum += processComb(combIndex, mono_in);
    sum *= 0.25f;

    for (uint32_t allpassIndex = 0; allpassIndex < kAllpassLineCount; ++allpassIndex)
      sum = processAllpass(allpassIndex, sum);

    // Slight stereo from delayed comb taps; keep it wet/deep, not dry.
    static const uint32_t kSideTap = 337U;
    const float side =
        comb_lines_[1][(rev_write_ + kCombSize - kSideTap) & kCombMask] * 0.35f;
    left_out = fastertanhf(sum + side);
    right_out = fastertanhf(sum - side);
    ++rev_write_;
  }
  ...
  {
    // Mild pitch drift around unison; SIZE bias leans slightly upward when deep.
    const float size_bias = (clamp01(size_norm_) - 0.5f) * 0.35f;
    const float roll = nextUnitRandom();
    float rate = 1.f;
    if (roll < 0.12f + size_bias)
      rate = 2.f;
    else if (roll > 0.88f - size_bias)
      rate = 0.5f;
  ...
  void spawnGrain()
  {
    ...
    // Prefer recent reverb bloom near the write tip.
    const float spray = 0.08f + smooth * 0.32f;
    const float center = 0.78f + randomSigned() * spray;
    ...
    if (overlap < 1.2f)
      overlap = 1.2f;
    const float density_gain = 1.7f / fasterpowf(overlap, 0.5f);
    const float velocity = (0.7f + nextUnitRandom() * 0.4f) * density_gain;
  ...
  void renderGrains(float &left, float &right)
  {
    ...
    for (uint32_t grainIndex = 0; grainIndex < kMaxGrains; ++grainIndex)
    {
      ...
      left += sample_left * amp * grain.pan_left;
      right += sample_right * amp * grain.pan_right;
    ...
    left = fastertanhf(left * 1.1f);
    right = fastertanhf(right * 1.1f);
  }
  ...
  float feel_norm_ = 0.65f;
  float size_norm_ = 0.75f;
  float mix_ = 1.f;
  float env_norm_ = 0.6f;
  float sprd_norm_ = 0.4f;
  float tone_norm_ = 0.45f;
  float revs_norm_ = 0.1f;
  ...
  uint32_t rng_state_ = 0xA11CE55U;
  Grain grains_[kMaxGrains];

  uint32_t rev_write_ = 0U;
  float comb_feedback_ = 0.92f;
  float damp_coeff_ = 0.35f;
  float damp_states_[kCombLineCount] = {};
};
