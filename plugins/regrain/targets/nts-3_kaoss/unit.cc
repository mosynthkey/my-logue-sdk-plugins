 ...
 * File: unit.cc
 *
 * NTS-3 generic effect unit interface for ReGrain
 */
...
static ReGrain s_regrain_instance;
...
__unit_callback int8_t unit_init(const unit_runtime_desc_t *desc)
{
  ...
  if (desc->samplerate != s_regrain_instance.getSampleRate())
  ...
  float *allocated_buffer =
      (float *)desc->hooks.sdram_alloc(s_regrain_instance.getBufferSize() * sizeof(float));
  if (!allocated_buffer)
    return k_unit_err_memory;

  std::fill(allocated_buffer, allocated_buffer + s_regrain_instance.getBufferSize(), 0.f);
  s_runtime_desc = *desc;
  ...
  s_regrain_instance.init(allocated_buffer);
  ...
  for (uint8_t paramIndex = 0; paramIndex < unit_header.common.num_params; ++paramIndex)
    s_regrain_instance.setParameter(paramIndex, cached_values[paramIndex]);
...
{
  s_regrain_instance.teardown();
  s_get_raw_input = nullptr;
}

__unit_callback void unit_reset()
{
  s_regrain_instance.reset();
}

__unit_callback void unit_resume()
{
  s_regrain_instance.resume();
}

__unit_callback void unit_suspend()
{
  s_regrain_instance.suspend();
}
...
__unit_callback void unit_render(const float *in, float *out, uint32_t frames)
{
  ...
  if (raw == nullptr && s_get_raw_input != nullptr)
    raw = s_get_raw_input();
  s_regrain_instance.process(in, raw, out, frames);
}
...
__unit_callback void unit_set_param_value(uint8_t id, int32_t value)
{
  ...
  s_regrain_instance.setParameter(id, value);
}
