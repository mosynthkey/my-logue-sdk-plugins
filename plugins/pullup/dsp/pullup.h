 ...
 * File: pullup.h
 *
 * Pull-up / rewind. Always records. Touch jumps to a cue; optional reverse
 * theater then plays forward from that point while held.
 */
...
public:
  ...
  void setParameter(uint8_t index, int32_t value) override final
  {
    ...
    {
    case CUE:
      cue_norm_ = param_10bit_to_f32(value);
      break;
    case WIND:
      wind_norm_ = param_10bit_to_f32(value);
      break;
    case MIX:
  ...
  void touchEvent(uint8_t, uint8_t phase, uint32_t, uint32_t) override final
  {
    if (phase == k_unit_touch_phase_began)
    {
      ...
      if (cue_norm_ < 0.33f)
        cue_pos_ = 0.f;
      else if (cue_norm_ < 0.66f)
        cue_pos_ = loop_f * 0.5f;
      else
        cue_pos_ = loop_f * 0.75f;

      if (wind_norm_ > 0.15f)
      {
        rewinding_ = true;
        rewind_left_ = wind_norm_ * 0.4f * getSampleRate();
        play_pos_ = loop_f - 1.f;
      }
      else
      {
        rewinding_ = false;
  ...
  void process(const float *__restrict in, const float *__restrict raw, float *__restrict out, uint32_t frames)
  {
    ...
    for (uint32_t sampleIndex = 0; sampleIndex < frames; ++sampleIndex)
    {
      ...
      if (playing_ && loop_len > 8U && captured_ > 64U)
      {
        ...
        readLoop(origin, play_pos_, loop_len, wet_left, wet_right);
...
private:
  ...
  float play_pos_ = 0.f;
  float cue_pos_ = 0.f;
  float rewind_left_ = 0.f;
  float bpm_ = 120.f;
  float cue_norm_ = 0.f;
  float wind_norm_ = 0.274f;
  float mix_ = 1.f;
  uint8_t bars_sel_ = 2;
  bool playing_ = false;
