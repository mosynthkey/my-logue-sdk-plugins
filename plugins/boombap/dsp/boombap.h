 ...
 * File: boombap.h
 *
 * Tempo-synced boom-bap kit for NTS-3.
 * Hold to run. X = ghost-note density. Y = hat / fill energy.
...
class BoomBap : public Processor
{
public:
...
private:
  ...
  // Heavy MPC-style 16th swing.
  float swingDelayFraction() const
  {
    const float swing = 0.5f + swing_norm_ * 0.4f;
    return swing * 0.32f;
  }
  ...
  // Boom on 1; late kick around the and-of-2 / beat 3.
  static bool isKickSpine(uint32_t step)
  {
    return step == 0U || step == 7U || step == 10U;
  }
  ...
  {
    return step == 2U || step == 3U || step == 5U || step == 6U || step == 9U || step == 11U ||
           step == 13U || step == 14U || step == 15U;
  }

  static bool isClosedHatSeat(uint32_t step)
  {
    return (step % 2U) == 0U || step == 3U || step == 7U || step == 11U || step == 15U;
  }
  ...
  {
    return fx::clip01(base * (0.78f + fx::randomFloat(rng_) * 0.4f));
  }
  ...
  void fire(HitKind kind, float velocity)
  {
    ...
    case kSnare:
      snare_age_ = 0.f;
      snare_vel_ = vel;
      snare_hz_ = 155.f + tone_norm_ * 80.f;
      snare_phase_ = 0.f;
      ++main_triggers_;
      break;
    case kGhost:
  ...
  {
    const bool fill_active = fill_timer_ > 0U || hats_norm_ > 0.9f;
    const float ghost = ghost_norm_;
    const float hats = hats_norm_;

    if (isKickSpine(step))
      fire(kKick, fill_active ? 1.f : (step == 0U ? 1.f : 0.82f));
    ...
    {
      float ghost_chance = ghost * 0.5f;
      if (step == 3U || step == 11U)
        ghost_chance += ghost * 0.4f;
      if (step == 5U || step == 13U)
        ghost_chance += ghost * 0.25f;
      if (fill_active)
        ghost_chance = fx::clip01(ghost_chance + 0.4f);
      if (fx::randomFloat(rng_) < ghost_chance)
        fire(kGhost, fx::clip01(0.16f + ghost * 0.2f + hats * 0.06f));
    }
    ...
    if (isClosedHatSeat(step))
    {
      ...
      if ((step % 2U) != 0U)
        hat_chance *= 0.55f + ghost * 0.35f;
      if (fill_active)
        hat_chance = 0.95f;
      if (fx::randomFloat(rng_) < hat_chance)
        fire(kHatClosed, ((step % 4U) == 0U) ? 0.62f : 0.35f);
    }
  ...
  float renderVoices()
  {
    ...
    const float kick_tau = (0.055f + decay_norm_ * 0.11f) * sr;
    const float snare_tau = (0.04f + decay_norm_ * 0.08f) * sr;
    const float ghost_tau = (0.02f + decay_norm_ * 0.03f) * sr;
    const float hat_c_tau = (0.014f + decay_norm_ * 0.022f) * sr;
    const float hat_o_tau = (0.07f + decay_norm_ * 0.1f) * sr;
    ...
    // Rounder boom kick with slower pitch drop.
    kick_hz_ += (34.f - kick_hz_) * 0.0016f;
    ...
    const float snare = (snare_tone * 0.28f + noise * 0.72f) * snare_env * 0.98f;

    // Dusty soft snare / rim ghosts.
    const float ghost = noise * ghost_env * 0.38f +
                        fastersinfullf(fx::wrap01(snare_phase_ * 1.55f) * kTwoPi) * ghost_env * 0.16f;

    const float hat_raw = noise;
    hat_hp_ += 0.32f * (hat_raw - hat_hp_);
    const float hat_bright = hat_raw - hat_hp_;
    const float hats = hat_bright * (hat_c_env * 0.28f + hat_o_env * 0.42f);

    return fx::softclip(kick + snare + ghost + hats);
  }

  float bpm_ = 90.f;
  float ghost_norm_ = 0.4f;
  float hats_norm_ = 0.3f;
  float swing_norm_ = 0.62f;
  float tone_norm_ = 0.4f;
