...
static void render(Trance &kit, std::vector<float> &mono, uint32_t frames)
{
...
static void setup(Trance &kit, int32_t hats, int32_t build)
{
  kit.init(nullptr);
  kit.setParameter(Trance::MIX, 1000);
  kit.setParameter(Trance::HATS, hats);
  kit.setParameter(Trance::BUILD, build);
  kit.setParameter(Trance::SWING, 200);
  kit.setParameter(Trance::TONE, 460);
  kit.setParameter(Trance::DEC, 460);
  kit.setTempo(138.f);
}
...
int main()
{
  ...
  kit.touchEvent(0, k_unit_touch_phase_began, 400U, 400U);
  kit.tempo4ppqnTick(5U); // step 4 = clap
  if (kit.debugMainTriggers() < 1U)
  {
    std::printf("host mid-bar tick missed clap: mains=%u\n", kit.debugMainTriggers());
    return 10;
  }
