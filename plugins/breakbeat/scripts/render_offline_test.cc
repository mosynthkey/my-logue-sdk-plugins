...
static void render(BreakBeat &kit, std::vector<float> &mono, uint32_t frames)
{
...
static void setup(BreakBeat &kit, int32_t sync, int32_t energy)
{
  kit.init(nullptr);
  kit.setParameter(BreakBeat::MIX, 1000);
  kit.setParameter(BreakBeat::SYNC, sync);
  kit.setParameter(BreakBeat::ENERGY, energy);
  kit.setParameter(BreakBeat::SWING, 360);
  kit.setParameter(BreakBeat::TONE, 510);
  kit.setParameter(BreakBeat::DEC, 410);
  kit.setTempo(174.f);
}
...
int main()
{
  ...
  // Amen-ish spine: kicks 0/2/6/10, snares 4/12.
  kit.debugTriggerStep(0U);
  kit.debugTriggerStep(2U);
  ...
  {
    std::printf("sync ghosts too low: %u\n", kit.debugGhostTriggers());
    return 2;
  }

  const uint32_t high_sync = kit.debugGhostTriggers();
  setup(kit, 80, 200);
  kit.debugResetCounters();
  for (uint32_t passIndex = 0; passIndex < 8U; ++passIndex)
  {
  ...
  const uint32_t low_sync = kit.debugGhostTriggers();
  if (low_sync >= high_sync)
  {
    std::printf("low sync=%u not below high=%u\n", low_sync, high_sync);
    return 3;
  }
  ...
  mono.clear();
  render(kit, mono, 1000U); // < one 16th at 174 BPM
  ...
  kit.touchEvent(0, k_unit_touch_phase_began, 400U, 400U);
  kit.tempo4ppqnTick(3U); // step 2 = kick
  ...
  std::printf("ok sync_high=%u sync_low=%u kick_peak=%f late_ratio=%f bar_peak=%f\n", high_sync, low_sync,
