 ...
 * File: breakbeat.h
 *
 * Tempo-synced Amen-inspired breakbeat kit for NTS-3 (synthetic, not a sample).
 * Hold to run. X = syncopation / ghost density. Y = break energy / fill.
...
class BreakBeat : public Processor
{
public:
  ...
  void setParameter(uint8_t index, int32_t value) override final
  {
    ...
    {
    case SYNC:
      sync_norm_ = param_10bit_to_f32(value);
      break;
    case ENERGY:
      energy_norm_ = param_10bit_to_f32(value);
      break;
    case MIX:
...
private:
  ...
  {
    const float swing = 0.2f + swing_norm_ * 0.28f;
    return swing * 0.2f;
  }
  ...
  // Amen-inspired kick map (synthetic approximation).
  static bool isKickSpine(uint32_t step)
  {
    return step == 0U || step == 2U || step == 6U || step == 10U;
  }
  ...
  // Amen-ish secondary snare / ghost cluster near the end of the bar.
  static bool isBreakSnareSeat(uint32_t step)
  {
    return step == 7U || step == 13U || step == 15U;
  }

  static bool isGhostSeat(uint32_t step)
  {
    return step == 1U || step == 3U || step == 5U || step == 8U || step == 9U || step == 11U ||
           step == 14U;
  }

  static bool isClosedHatSeat(uint32_t step)
  {
    return (step % 2U) == 0U || step == 1U || step == 5U || step == 9U || step == 11U;
  }

  static bool isOpenHatSeat(uint32_t step)
  {
    return step == 6U || step == 14U;
  }
  ...
  void fire(HitKind kind, float velocity)
  {
    ...
    case kSnare:
      snare_age_ = 0.f;
      snare_vel_ = vel;
      snare_hz_ = 175.f + tone_norm_ * 95.f;
      snare_phase_ = 0.f;
      ++main_triggers_;
      break;
    case kGhost:
  ...
  {
    const bool fill_active = fill_timer_ > 0U || energy_norm_ > 0.9f;
    const float sync = sync_norm_;
    const float energy = energy_norm_;

    if (isKickSpine(step))
      fire(kKick, fill_active ? 1.f : (step == 0U ? 1.f : 0.78f));
    ...
    if (isSnareSpine(step))
      fire(kSnare, fill_active ? 1.f : 0.94f);
    else if (isBreakSnareSeat(step))
    {
      float snare_chance = 0.35f + sync * 0.55f + energy * 0.15f;
      if (step == 7U)
        snare_chance += 0.25f;
      if (fill_active)
        snare_chance = 0.95f;
      if (fx::randomFloat(rng_) < snare_chance)
        fire(kSnare, step == 7U ? 0.7f : 0.4f + sync * 0.35f);
    }
    else if (fill_active && fx::randomFloat(rng_) < 0.85f)
      fire(kSnare, 0.5f + fx::randomFloat(rng_) * 0.35f);

    if (!isKickSpine(step) && !isSnareSpine(step) && isGhostSeat(step))
    {
      float ghost_chance = sync * 0.55f;
      if (step == 3U || step == 11U || step == 14U)
        ghost_chance += sync * 0.3f;
      if (fill_active)
        ghost_chance = fx::clip01(ghost_chance + 0.4f);
      if (fx::randomFloat(rng_) < ghost_chance)
        fire(kGhost, fx::clip01(0.18f + sync * 0.22f));
    }
    ...
    if (isClosedHatSeat(step))
    {
      ...
      if ((step % 2U) != 0U)
        hat_chance *= 0.7f + sync * 0.3f;
      if (fill_active)
        hat_chance = 0.96f;
      if (fx::randomFloat(rng_) < hat_chance)
        fire(kHatClosed, ((step % 4U) == 0U) ? 0.65f : 0.38f);
    }
    ...
    if (isOpenHatSeat(step))
    {
      ...
      if (fx::randomFloat(rng_) < open_chance)
        fire(kHatOpen, 0.45f + energy * 0.3f);
    }
  }
  ...
  float renderVoices()
  {
    ...
    const float kick_tau = (0.035f + decay_norm_ * 0.07f) * sr;
    const float snare_tau = (0.03f + decay_norm_ * 0.06f) * sr;
    const float ghost_tau = (0.016f + decay_norm_ * 0.025f) * sr;
    const float hat_c_tau = (0.011f + decay_norm_ * 0.018f) * sr;
    const float hat_o_tau = (0.055f + decay_norm_ * 0.09f) * sr;
    ...
    kick_hz_ += (40.f - kick_hz_) * 0.0028f;
    kick_phase_ = fx::wrap01(kick_phase_ + kick_hz_ / sr);
    const float kick = fastersinfullf(kick_phase_ * kTwoPi) * kick_env * 1.3f;
    ...
    const float snare = (snare_tone * 0.3f + noise * 0.7f) * snare_env * 1.0f;

    const float ghost = noise * ghost_env * 0.4f +
                        fastersinfullf(fx::wrap01(snare_phase_ * 1.8f) * kTwoPi) * ghost_env * 0.2f;

    const float hat_raw = noise;
    hat_hp_ += 0.38f * (hat_raw - hat_hp_);
    const float hat_bright = hat_raw - hat_hp_;
    const float hats = hat_bright * (hat_c_env * 0.3f + hat_o_env * 0.48f);

    return fx::softclip(kick + snare + ghost + hats);
  }

  float bpm_ = 174.f;
  float sync_norm_ = 0.4f;
  float energy_norm_ = 0.3f;
