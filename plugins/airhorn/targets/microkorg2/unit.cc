 ...
 * File: unit.cc
 *
 * microKORG2 oscillator unit interface for AirHorn
 */
...
#include "airhorn_mk2.h"

static AirHornMk2 s_airhorn_instance;
...
__attribute__((used)) int8_t unit_init(const unit_runtime_desc_t *desc)
{
  ...
  s_runtime_desc = *desc;
  return s_airhorn_instance.Init(desc);
}
...
{
  (void)in;
  s_airhorn_instance.Process(out, frames);
}
...
{
  return s_airhorn_instance.getParameterValue(id);
}
