...
## NTS-3 Hardware Pitfalls (HSnare notes)
...
### 2. HSnare (2026-09): not a link error

Built `hsnare.elf` has **no undefined libm symbols**. PLT entries are only
local methods + `memset`. HSnare has `ULIBS =` empty. Link resolution is fine.
...
- Hardcoded multiply decays such as `0.988` / `0.992` (~12–18 ms to −60 dB)
  also sound like attack clicks; that is an envelope length issue, not a link
  issue.
